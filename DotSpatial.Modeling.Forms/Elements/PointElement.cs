...
{
    /// <summary>
    /// Point Element for use in the tool dialog
    /// </summary>
    internal partial class PointElement : DialogElement
    {
        #region Fields
        ...
        private void BtnAddDataClick(object sender, EventArgs e)
        {
            ...
            if (tempFeatureSet.FeatureType != FeatureType.Point)
            {
        ...
        private void DoRefresh()
        {
            ...
            if (_dataSets != null)
            {
                foreach (DataSetArray dsa in _dataSets)
                {
                    ...
                    if (aFeatureSet?.FeatureType == FeatureType.Point && !comboFeatures.Items.Contains(dsa))
                    {
