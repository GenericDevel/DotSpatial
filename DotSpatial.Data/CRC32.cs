...
{
    /// <summary>
    /// Implementation of the Cyclic redundancy check CRC 32.
    /// </summary>
    public static class Crc32
    {
        ...
        private static uint[] createdTable;
        ...
        /// </summary>
        /// <param name="bytes">Bytes used for the calculation.</param>
        /// <returns>The calculated checksum.</returns>
        public static long ComputeChecksum(byte[] bytes)
        {
            if (table == null) table = CreateTable2();
            uint crc = 0xffffffff;
            for (int i = 0; i < bytes.Length; ++i)
            {
                byte index = (byte)((crc & 0xff) ^ bytes[i]);
                crc = (crc >> 8) ^ (uint)table[index];
            }
        ...
        /// </summary>
        /// <returns>The created array of checksum values.</returns>
        ...
        {
            uint crcVal = value.Aggregate(0xffffffff, (current, t) => (current >> 8) ^ CrcTable[(current & 0xff) ^ t]);
        ...
        private static uint UpdateCrc(uint crc, byte[] buff, int len)
        {
            uint c = crc;
            int n;
            if (createdTable == null) createdTable = CreateTable();
            for (n = 0; n < len; n++)
            {
                c = createdTable[(c ^ buff[n]) & 0xff] ^ (c >> 8);
            }
